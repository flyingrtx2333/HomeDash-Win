...
| **Favicon 自动抓取** | 输入端口后自动获取目标服务的网站图标 |
| **图标自定义** | 支持上传图片、拖拽上传、Emoji 选择，自定义服务图标 |
| **服务启动/停止** | 一键启动和停止服务，支持启动命令和进程名配置 |
...
| **温度监控** | 实时显示 CPU 和 GPU 温度，高温预警 |
| **数据可视化图表** | 监控页面支持 CPU、内存、GPU、网络流量实时曲线图，自动绘制历史数据 |
| **时间范围选择** | 支持 1小时/6小时/24小时/7天 时间范围切换，查看历史趋势 |
| **磁盘使用详情** | 监控页面显示详细磁盘使用情况，支持百分比进度条和颜色标识 |
| **系统信息概览** | 显示操作系统、运行时间、进程数量、系统负载等系统信息 |
| **进程管理** | 查看系统进程列表，按 CPU/内存占用排序（Top 20） |
...
| 📋 **日志查看器** | 实时查看各服务日志，支持按级别过滤、自动刷新、清空日志 |
| 🔧 **增强进程管理** | 支持进程搜索、筛选、排序、批量操作、详情查看等功能 |
| 🦞 **OpenClaw 入口** | 内置 OpenClaw AI 助手服务模板，端口18789 |
|网站项目管理|
|Mysql数据库管理|
---
